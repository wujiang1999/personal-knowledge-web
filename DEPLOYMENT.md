...
- 2026-09-24（公式定界符兼容，97804a0）：上一批上线后用户仍见大量公式不显示——**根因是语料
  与解析器的定界符语言不一致**：知识库公式绝大多数来自论文/读书笔记/对话原文，用的是 LaTeX 原生
  定界符 `\(…\)`（行内）与 `\[…\]`（块级），只有少数条目用 `$…$` / `$$…$$`；`remark-math`
  只认美元符号。更棘手的是 CommonMark 在生成语法树**之前**就把 `\` 当转义吃掉（`\(` → 纯文本
  `(`），等 remark 插件运行时定界符已不可恢复。**修法**——新增纯函数模块
  `lib/markdown-source.ts` 的 `normalizeMathDelimiters`，在源码层（Markdown 解析前）改写定界符，
  两个渲染器（`concept-body.tsx` 详情/嵌入/ask 答案、`markdown-preview.tsx` 编辑器预览）各调用一次；
  围栏代码块与行内代码跨度内的 `\(…\)` 原样保留（`` `\(x\)` `` 仍显示源码），`\\` 视为
  `aligned` 环境的换行内容而非闭合定界符。**顺带**——独占行的 `\[…\]` 与单行 `$$…$$` 归一为规范
  块级形式（闭合栅栏回缩到开栅栏缩进，避免块级公式掉出列表项），此前这类公式虽能显示但按行内
  排版、不居中。**已知边界**：4 空格缩进代码块未识别（无完整块级解析器时其缩进与列表续行不可
  区分），其中的定界符会被改写；实际语料用的是围栏代码块。**验收**：新增 `tests/markdown-source.test.ts`
  13 条契约测试（行内/块级/列表缩进/`\\[2pt]`/围栏/行内代码/转义反斜杠/未闭合/单行 `$$` 提升）；
  线上真实正文片段跑完整管线：DQN 0→4 个公式、TD aligned 0→1、贝尔曼单行块级 0→1，美元写法与
  代码/货币边界无回退；`npm run check` 339 测试 + 生产构建过；ego lite 本地真实页面浅/深色目检
  （两渲染器各 10 个 katex 节点、各 5 个块级公式、字体实载、0 红色错误、代码块保持源码、
  代码与 MathML 之外无残留定界符）。服务器 bundle FF + `deploy.sh` 门禁 + 公网探针见下条。
